...
// C2：三个转场入口已改成 `pushUndo({ label: \`加转场 #…\`, kind: "transition", … })`。
// 用非贪婪的 `{…}` 跨度匹配标签，仍然**按标签逐条数**（总数相等不等于三处都在，
// 可能是加了一处新的、丢了一处旧的）。
ok("转场的增删改都进撤销栈",
  ["加转场", "转场时长", "删除转场"].every((t) =>
    new RegExp("pushUndo\\(\\{[\\s\\S]{0,400}?label: `" + t).test(app)),
  "3.6 之前转场是全项目唯一一处「改了就回不去」的编辑操作");
ok("三个转场入口都标了 kind: \"transition\"（不只是恰好都有 label）",
  (app.match(/kind: "transition"/g) ?? []).length === 3,
  "kind 是命令面板/统计读的字段，漏标的话那条命令会被归到「其他」里，功能不坏但统计失真");
